...
class Preprocessor(ABC):
        ...
        if ast.type == clingo.ast.ASTType.Rule:
            ...
            for control_object in [self._candidates_gen, self._candidates_test]:
                with control_object.builder() as builder:
                    builder.add(rule)
            if self._optimization > 1 and \
                    ast.head.type != clingo.ast.ASTType.Disjunction and \
                    ast.head.type != clingo.ast.ASTType.Aggregate:
    ...
    def _get_aux2_literal(self, literal):
        if literal.atom.type != clingo.ast.ASTType.SymbolicAtom:
            return literal
